...
document.addEventListener('input', function(event) {
  ...
  if (target && target.matches('.howmany')) {
    ...
    // let changedMultipleTimes = false; // initialize flag
    setTimeout(() => {
      // if (changedMultipleTimes) {
        // only reload if quantity was not changed multiple times
        location.reload();
      }
    }, 300);
    ...
    // // check if quantity was changed multiple times within 300ms
    // setTimeout(() => {
    //   if (parseInt(target.value) !== cart[index].totalQuantity) {
    //     changedMultipleTimes = true;
    //   }
    // }, 500); // adjust time frame as needed
  };
});
